...
## Unreleased
* Breaking change: Registry flag constants are re-exported with their generated `i32` types. Permission parameters use Microsoft's generated `REGSAM` type (also exported from `winreg::enums`), and option parameters use `u32`; cast signed constants at call sites.
* Replace the direct `windows-sys` dependency with focused, checked-in bindings generated by `windows-bindgen` 0.100.0 and linked with `windows-link` 0.100.0.
* Breaking change: Increase the minimum supported Rust version to 1.95 and use the 2024 edition.
* Breaking change: `FileTime` dereferences to winreg's own `FILETIME`, and `get_last_write_time_system()` returns winreg's own `SYSTEMTIME`. Both types are exported from `winreg::reg_key_metadata` and retain their Windows API field names and layout.
