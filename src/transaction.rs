...
impl Transaction {
    ...
    pub fn new() -> io::Result<Transaction> {
        unsafe {
            let handle = bindings::CreateTransaction(
                ptr::null_mut(),
                ptr::null_mut(),
                0,
                0,
    ...
    pub fn commit(self) -> io::Result<()> {
        unsafe {
            match bindings::CommitTransaction(self.handle) {
    ...
    pub fn rollback(self) -> io::Result<()> {
        unsafe {
            match bindings::RollbackTransaction(self.handle) {
                0 => Err(io::Error::last_os_error()),
                _ => Ok(()),
            }
        }
    ...
    fn close_(&mut self) -> io::Result<()> {
        unsafe {
            match bindings::CloseHandle(self.handle) {
                0 => Err(io::Error::last_os_error()),
                _ => Ok(()),
            }
        }
